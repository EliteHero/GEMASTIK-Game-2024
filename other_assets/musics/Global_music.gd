...
@onready var hallway_music = $hallway_music
...
func play_hallway():
	if not hallway_music.playing == true:
		hallway_music.volume_db = 0
		hallway_music.play()
...
func stop_hallway():
	if hallway_music and hallway_music.playing:
		animation.play("hallway_music_off")
		await get_tree().create_timer(1).timeout
		hallway_music.stop()
